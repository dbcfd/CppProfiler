...
namespace profiling {
    ProfilerStruct::ProfilerStruct() : programName(0), commandLine(0)
    {
...
#else
		Profiler.programName = copystring( "only_for_win32" );
		Profiler.commandLine = copystring( "" );
#endif
	}
	...
	*/
	...
	struct HTMLDumper {
		void Init() {
			...
			snprintf( fileFormat, 255, "%s-profile-%s.html", Profiler.programName ? Profiler.programName : "no-info-given", timeFormat );
			strftime( timeFormat, 255, "%#c", now_tm );			
			f = fopen( fileFormat, "w" );
	...
	struct MakeRoot {
		...
		~MakeRoot() {
			free( Profiler.programName );
			free( Profiler.commandLine );
		}
	};
    static MakeRoot makeRoot;

    void ProfilerStruct::detect( int argc, const char *argv[] ) { detectByArgs( argc, argv ); }
	void ProfilerStruct::detect( int argc, char *argv[] ) { detectByArgs( argc, argv ); }
	void ProfilerStruct::detect( const char *commandLine ) { detectWinMain( commandLine ); }
	void ProfilerStruct::dump() { dumpThreads( PrintfDumper() ); }
    std::string ProfilerStruct::dumpstring() { 
        StringDumper dumper;
        dumpThreads( dumper ); 
        return dumper.output.str();
        return std::string();
    }
	void ProfilerStruct::dumphtml() { dumpThreads( HTMLDumper() ); }
	void fastcall ProfilerStruct::enter( const char *name ) { enterCaller( name ); }
	void fastcall ProfilerStruct::exit() { exitCaller(); }
	void fastcall ProfilerStruct::pause() { pauseCaller(); }
	void fastcall ProfilerStruct::unpause() { unpauseCaller(); }
	void ProfilerStruct::threadenter( const char *name ) { enterThread( name ); }
	void ProfilerStruct::threadexit() { exitThread(); }
	void ProfilerStruct::reset() { resetThreads(); }
